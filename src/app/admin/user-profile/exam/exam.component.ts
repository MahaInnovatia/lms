...
ngOnInit(): void {
  const tutorialData = this.dataService.getData('examData');
  if (tutorialData) {
    const { studentId, courseId, courseName } = tutorialData;
    this.getExamAnswers(studentId, courseId.id);
...
getExamAnswers(studenId: string, courseId: string){
  this.alluserService.getExamAnswers(studenId, courseId).subscribe(answers =>{
