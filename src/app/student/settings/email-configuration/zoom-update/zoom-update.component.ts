...
@Component({
  selector: 'app-zoom-update',
  templateUrl: './zoom-update.component.html',
  styleUrls: ['./zoom-update.component.scss']
})
export class ZoomUpdateComponent {
  edit = true;
  ...
  getForgetPasswordTemplate() {
    this.emailConfigurationService.getForgetPasswordTemplate().subscribe( response =>{
      this.assignData  = response?.data?.docs[0]?.zoom_meeting_updation_template;
...
update(){
  ...
  return new Promise<void>((resolve, reject) => {
        ...
        const payload = {
          ...
          insertaction: 'zoom_meeting_updation_template'
        }
        obj.insertaction = 'zoom_meeting_updation_template';
